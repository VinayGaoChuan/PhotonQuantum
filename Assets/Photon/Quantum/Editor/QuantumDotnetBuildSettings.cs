namespace Quantum.Editor {
  ...
  public class QuantumDotnetBuildSettings : QuantumGlobalScriptableObject<QuantumDotnetBuildSettings> {
    ...
    public static void GenerateProject(QuantumDotnetBuildSettings settings, bool disablePopup = false) {
      ...
      string slnName = Path.GetFileName(settings.ProjectBasePath);
      // Create the solution file
      if (File.Exists($"{settings.ProjectBasePath}/{slnName}.sln") == false) {
        RunDotnetCommand($" new sln --output {slnName}", settings.DotnetCommandPath);
        RunDotnetCommand($" sln {settings.ProjectBasePath}/{slnName}.sln add {settings.ProjectBasePath}/Quantum.Simulation.Dotnet", settings.DotnetCommandPath);
        RunDotnetCommand($" sln {settings.ProjectBasePath}/{slnName}.sln add {settings.ProjectBasePath}/Quantum.Runner.Dotnet", settings.DotnetCommandPath);
      }
