...
public class StructWrapperPools
{
	...
	private StructWrapperPool<T> GetPoolForType<T>()
	{
		...
		{
			return existingpool as StructWrapperPool<T>;
		}
		StructWrapperPool<T> newpool = new StructWrapperPool<T>(isStaticPool: false);
	...
	{
		StructWrapper<T> item = GetPoolForType<T>().Acquire(value);
		used.Add(item);
		return item;
	}
